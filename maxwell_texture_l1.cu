/*
   Finds: Maxwell unified texture / L1 cache
...
__global__ void texture_latency (int* my_array, unsigned int *duration, int *index) {
...
void parametric_measure_texture(int N, int stride) {
  ...
  // launch kernel
  texture_latency<<<Dg, Db>>>(d_a, d_duration, d_index);
  ...
  printf("\n=====Visting the %f KB array, loop %d*%d times======\n", (float)(N)*sizeof(int)/1024.0f, SHARED_LEN, 1);
  for (int i = 0; i < SHARED_LEN; i++){ 
